...
export class AdaptRuleEngine {
  ...
  evaluate(input: EngineInput): EngineOutput {
    ...
    this.trace.push(
      `→ Cognitive scores: WM=${cognitiveScores.workingMemory}, ATN=${cognitiveScores.attention}, SPD=${cognitiveScores.processingSpeed}`
    );
    ...
    const { severity, confidence } = this.assessSeverityAndConfidence(
      taskState,
      cognitiveScores,
      input.historySignals.recentFailureCount,
      input.taskContext.riskLevel
    );
    this.trace.push(`→ Severity: ${severity}, Confidence: ${confidence}%`);

    // Step 5: Select assistance mode
    const { assistanceMode, adaptationActions, escalationRequired, escalationReason } =
      this.selectAssistanceMode(taskState, severity, input.taskContext.riskLevel);
  ...
  private determineTaskState(input: EngineInput): TaskState {
    ...
    if (
      passiveSignals.reminderDelivered &&
    ...
    if (
      interactionSignals.instructionReplay >= 2 ||
  ...
  private scoreCognitiveDifficulty(input: EngineInput): CognitiveScores {
    ...
    Object.keys(scores).forEach((key) => {
      scores[key as keyof CognitiveScores] = Math.min(
        100,
        scores[key as keyof CognitiveScores]
      );
    });
  ...
  private identifyPrimaryIssues(
    ...
    const primaryIssue =
      scores[sorted[0] as keyof CognitiveScores] > 30 ? (sorted[0] as keyof CognitiveScores) : "NONE";
    const secondaryIssue =
      scores[sorted[1] as keyof CognitiveScores] > 20 ? (sorted[1] as keyof CognitiveScores) : "NONE";
  ...
  private selectAssistanceMode(
    ...
    let assistanceMode: AssistanceMode = "NONE";
    let adaptationActions: string[] = [];
